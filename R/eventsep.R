eventsep <- function(dailyMQ, monthlyHQ=NULL,dvar=3,theta=0.25, ddur=40,
		...
		if(length(to_int)>=1){
			for(i in 1:length(to_int)){
				...
				daten[inds_to_interp,2] <- seq(daten[inds_to_interp[1],2],
					daten[inds_to_interp[length(inds_to_interp)],2],length.out=length(inds_to_interp))
			}
		}
		cat("Interpolated",  length(to_int), "NA-gaps\n")
	...
	for(list_it in 1:length(daten_list)){
		...
		if(list_it==1){
			...
			events_all <-rbind(events_all,events2)
		}
	}
