  ...
  eventsep<-function(dailyMQ,monthlyHQ,dvar=3,sharevar=0.25, durday=40, usemed=FALSE, medbf=0.5){
  ...
  thvar<-mean(var3d[dvar:length(var3d)], na.rm = TRUE)+sharevar*sd(var3d[dvar:length(var3d)], na.rm = TRUE)
  ...
  while(i <= length(var3d)){
    ...
    if(is.na(var3d[i])){}else{
      if(var3d[i]>thvar){
        ...
        while(((daten[pos_start+1,2]-daten[pos_start,2])< ((daten[peak_ind,2]-daten[pos_start,2])/100*10)) && (pos_start<=peak_ind)) {
          pos_start<-pos_start+1
        }

        if(((daten[pos_start-1,2]-daten[pos_start-2,2])>((daten[peak_ind,2]-daten[pos_start,2])/100*40)) && (daten[pos_start-2,2]<daten[peak_ind,2])){
        ...
        if(usemed){
        while( (((((sum(diffs[(peak_ind+1):(pos_end+2)], na.rm = TRUE)-sum(diffs[(peak_ind+1):(pos_end)], na.rm = TRUE))/sum(diffs[(peak_ind+1):(pos_end)], na.rm = TRUE))>1.2)|| any(base_diff>0) || (base_rel>(2*medbf)) || ((incsum+sum(diffs[(peak_ind+1):pos_end], na.rm = TRUE))>(1*daten[pos_start,2])))) ){
          ...
          while( (((((sum(diffs[(peak_ind+1):(pos_end+2)], na.rm = TRUE)-sum(diffs[(peak_ind+1):(pos_end)], na.rm = TRUE))/sum(diffs[(peak_ind+1):(pos_end)], na.rm = TRUE))>1.2)|| any(base_diff>0)  || ((incsum+sum(diffs[(peak_ind+1):pos_end], na.rm = TRUE))>(1*daten[pos_start,2])))) ){
            if(is.na(cumdiffs[pos_end+1])){break}
            pos_end<-pos_end+1
          }
        }
